...
@section('title', '神器导航')
@section('keywords', \App\Wen\Utils\Settings::get('app_title').'神器导航')
...
@section('main')
    ...
    <main class="pb-30">
        ...
        <div class="archive-header pt-50 text-center">
            <div class="container">
                <h2 class="font-weight-900">互联网工作者的实用工具库</h2>
                <div class="breadcrumb mb-20">
                    一个好的「工具」能让你以最高效的方式完成工作
                </div>
                        ...
                        @foreach ($item->list as $items)
                            <article class="col-md-3 mb-30 wow">
                                <div class="hover-tt3 bg-white border-radius-10">
                                    <a href="{{ $items->link }}" target="_blank" class="tool-box">
                                        <img class="img-ofc tool-icon" src="{{ $items->icon }}"/>
                                        <h5 class="wlc1 mt-10">{{ $items->name }}</h5>
